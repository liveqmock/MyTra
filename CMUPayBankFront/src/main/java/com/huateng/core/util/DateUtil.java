	...
	public static String paseLog(String xmlContent){
		...
		try {
			...
			if(xmlContent.indexOf("<UserName>")!=-1){
				...
				String startXml=xmlContent.substring(0,xmlContent.indexOf("<UserName>"));
				xmlBody.append(startXml);
				xmlBody.append(userName);
				String endXml=xmlContent.substring(xmlContent.indexOf("</UserName>")+11);
				xmlBody.append(endXml);
				xmlContent=xmlBody.toString();
			}
			if(xmlContent.indexOf("<BankAcctID>")!=-1){
				xmlBody=new StringBuffer();
				String startStr=xmlContent.substring(0,xmlContent.indexOf("<BankAcctID>"));
				xmlBody.append(startStr);
				xmlBody.append(bankAcctID);
				String endXml=xmlContent.substring(xmlContent.indexOf("</BankAcctID>")+13);
				xmlBody.append(endXml);
				xmlContent=xmlBody.toString();
			}
			if(xmlContent.indexOf("<UserID>")!=-1){
				xmlBody=new StringBuffer();
				String startStr=xmlContent.substring(0,xmlContent.indexOf("<UserID>"));
				xmlBody.append(startStr);
				xmlBody.append(userId);
				String endXml=xmlContent.substring(xmlContent.indexOf("</UserID>")+9);
				xmlBody.append(endXml);
				xmlContent=xmlBody.toString();
			}
		} catch (Exception e) {
