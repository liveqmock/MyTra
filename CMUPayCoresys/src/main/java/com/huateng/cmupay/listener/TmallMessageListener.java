		...
		try {
			...
			if (message instanceof TextMessage) {
				...
				try {
					...
					tmallLogger.info("接收到天猫报文,交易流水ReqTransID:{},内部交易流水:{},天猫机构号:{}", new Object[] {
							msgVo.getReqTransID(), msgVo.getTxnSeq(),msgVo.getReqSys()});
				}catch (Exception e) {
					tmallLogger.error("接收到天猫发起交易流水ReqTransID:{},内部交易流水:{},报文为:{},报文头解析错误:{}",
				...
				if (validateRtn == null || "".equals(validateRtn)) {
					tmallLogger.debug("天猫发起方流水:{},内部流水:{},天猫机构号:{},报文头校验成功",
							new Object[] {msgVo.getReqTransID(), msgVo.getTxnSeq(),msgVo.getReqSys()});
				} else {
					tmallLogger.warn("天猫发起方流水:{},内部流水:{},天猫机构号:{},报文头校验成功:{}",
				...
				try {
					...
					if (result == null) {
						...
						if (code == null
								|| "".equals(StringUtil.toTrim(code
										.getTransCode()))) {
							tmallLogger.warn("发起方机构没有此服务的权限,天猫发起方流水:{},内部流水:{},天猫机构号:{},交易码:{}",
						...
						tmallLogger.warn("天猫发起方流水:{},内部流水:{},天猫机构号:{},交易码:{},内部交易码:{}",
					...
					tmallLogger.error("未知异常", e);
					tmallLogger.error("天猫发起方流水:{},内部流水:{},天猫机构号:{},未知异常:{}",new Object[] {msgVo.getReqTransID(), msgVo.getTxnSeq(), msgVo.getReqSys(),e.getMessage()});
				...
				if (replyToDest != null) {
					...
					tmallLogger.info("天猫交易完成,发起流水ReqTransID:{},内部交易流水:{},应答码:{},天猫机构号:{},内部交易码:{}",
							new Object[] { msgVo.getReqTransID(),msgVo.getTxnSeq(), resultCode,msgVo.getReqSys(),transCode });
				}

			}
		} catch (Exception e) {
			tmallLogger.error("系统错误", e);
