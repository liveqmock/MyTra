...
//					.getDateyyyyMMddHHmmssSSS();
			...
			initLog(txnLog, seqId, transIDHTime, intTxnDate,
			...
			if (!"".equals(StringUtil.toTrim(checkrtn))) {
				...
				txnLog.setChlRspDesc(RspCodeConstant.Bank.BANK_014A04.getDesc()
						+ checkrtn);
				txnLog.setChlRspType(RspCodeConstant.Bank.BANK_014A04.getValue());
				txnLog.setChlSubRspCode(RspCodeConstant.Bank.BANK_014A04.getDesc()
						+ checkrtn);
...
//						.getValue());
			...
			if(StringUtils.isBlank(forwardOrg)){
				...
				logger.warn("银行交易结果查询接口!原交易未发送到省充值:{},内部交易流水:{},发起方:{}",
						new Object[] { reqBody.getOriReqTransID(), transIDH,reqMsg.getReqSys() });
				resBody.setOriRspCode(transLog.getChlSubRspCode());
				resBody.setOriRspDesc(transLog.getChlSubRspDesc());
				resMsg.setBody(resBody);
				logger.debug("BankTransQueryAction execute(Object) - end");
				return resMsg;
			}
...
//			forwardBody.setOriReqSys(transLog.getRcvDomain());
			forwardBody.setOriReqSys(transLog.getReqDomain());
...
//						.getValue());
				...
				resMsg.setRspCode(resBody.getOriRspCode());
				resMsg.setRspDesc(resBody.getOriRspDesc());
