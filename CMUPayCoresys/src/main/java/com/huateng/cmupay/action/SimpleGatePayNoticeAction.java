...
public class SimpleGatePayNoticeAction extends
	...
	public CoreResultPayRes execute(Object param) throws AppBizException {
		...
		try {
			reqMsg = JacksonUtils.json2Bean(param.toString(),CoreResultPayReq.class);
			...
			if (null == logTmp) {
				...
				resMsg.setRspInfo(RspCodeConstant.Gate.GATE_9999.getDesc()+ "fail原支付交易不存在");
				return resMsg;
			}
			...
			if (null == txnLog) {
				...
				logger.info("网关支付结果通知,订单号: {} ,银行支付结果通知状态： {}", reqMsg.getOrderID(),txnLog.getStatus());
				if (CommonConstant.TxnStatus.TxnSuccess.getValue().equals(txnLog.getStatus())) {
					logger.info("网关支付结果通知,订单号: {} ,网关支付后台结果通知为成功 ", reqMsg.getOrderID());
					log.succ("网关支付结果通知,订单号: {} ,网关支付后台结果通知为成功 ", reqMsg.getOrderID());
				} else {
					logger.error("网关支付结果通知,订单号: {} ,网关支付后台结果通知为失败,或者未通知 ", reqMsg.getOrderID());
					log.error("网关支付结果通知,订单号: {} ,网关支付后台结果通知为失败 ,或者未通知", reqMsg.getOrderID());
				}
			}
			...
			log.error("网关支付结果通知,内部异常!内部流水:{}", new Object[] { logTmp.getIntTxnSeq() });
