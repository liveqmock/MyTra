...
/*			DataSourceContextHolder.setDataSourceType(DataSourceInstances.DS_NOW);
			...
			if (upayLog != null) {
				logger.info("移动渠道缴费退费,交易流水表中有原缴费记录:{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
						new Object[]{reqVo.getOriTransactionID(),reqVo.getTransactionID(),transIDH,
						paramData.getMsgSender(),upayLog.getIdValue()});
				log.warn("移动渠道缴费退费,交易流水表中有原缴费记录:{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
						new Object[]{reqVo.getOriTransactionID(),reqVo.getTransactionID(),transIDH,
						paramData.getMsgSender(),upayLog.getIdValue()});
//				String mainOrgId = SysMapCache.getProvCd(upayLog.getIdValue()).getSysCd();// 手机号码充值归属省(不管是主号、还是副号,此时转发的机构都是这个)
				String mainOrgId = upayLog.getRcvDomain();// 手机号码充值归属省(不管是主号、还是副号,此时转发的机构都是这个)
...
//					rspVo.setRspCode(upayLog.getChlSubRspCode());
//					rspVo.setRspInfo(upayLog.getChlSubRspDesc());
					rspVo.setTransactionID(reqVo.getTransactionID());
					msgRtn.setBody(rspVo);
...
//				if (orgFlag == false) {
//					logger.info("落地方CRM机构权限关闭:{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
//							new Object[]{mainOrgId,reqVo.getTransactionID(),transIDH,
//							paramData.getMsgSender(),upayLog.getIdValue()});
//					log.warn("落地方CRM机构权限关闭:{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
//							new Object[]{mainOrgId,reqVo.getTransactionID(),transIDH,
//							paramData.getMsgSender(),upayLog.getIdValue()});
//					txnLog.setChlRspCode(RspCodeConstant.Wzw.WZW_2998.getValue());
//					txnLog.setChlRspDesc(RspCodeConstant.Wzw.WZW_2998.getDesc());
...
//				String bankID = upayLog.getBankId();
				if(StringUtils.isBlank(bankID)){
					logger.warn("移动渠道缴费退费,原交易未发往银行进行扣款:{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
							new Object[]{reqVo.getOriTransactionID(),reqVo.getTransactionID(),transIDH,
							paramData.getMsgSender(),upayLog.getIdValue()});
					log.warn("移动渠道缴费退费,原交易未发往银行进行扣款:{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
							new Object[]{reqVo.getOriTransactionID(),reqVo.getTransactionID(),transIDH,
							paramData.getMsgSender(),upayLog.getIdValue()});
...
//				boolean orgFls = isO2OTransOn(paramData.getMsgSender(),bankID,mainOrgId,paramData.getTransCode().getTransCode());
				...
				addInitLg(txnLog, upayLog);
				logger.info("移动渠道缴费退费,成功添加到交易流水表 :{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
						new Object[]{upayLog.getIntTxnSeq(),reqVo.getTransactionID(),transIDH,
						paramData.getMsgSender(),upayLog.getIdValue()});
				log.warn("移动渠道缴费退费,成功添加到交易流水表 :{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
						new Object[]{upayLog.getIntTxnSeq(),reqVo.getTransactionID(),transIDH,
						paramData.getMsgSender(),upayLog.getIdValue()});
				
				if ((CommonConstant.TxnStatus.TxnSuccess.toString().equals(upayLog.getStatus()) || CommonConstant.TxnStatus.TxnFail.toString().equals(upayLog.getStatus()))) {
					logger.info("移动渠道缴费退费,原交易状态为00/01先向CRM发起冲正(返销) :{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
							new Object[]{reqVo.getOriTransactionID(),reqVo.getTransactionID(),transIDH,
							paramData.getMsgSender(),upayLog.getIdValue()});
					log.warn("移动渠道缴费退费,原交易状态为00/01先向CRM发起冲正(返销) :{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
							new Object[]{reqVo.getOriTransactionID(),reqVo.getTransactionID(),transIDH,
							paramData.getMsgSender(),upayLog.getIdValue()});
					
					if(!(CommonConstant.BussType.OnlineConsumeBus.toString().equals(upayLog.getBussType())||CommonConstant.BussType.OnlineChargeBus.toString().equals(upayLog.getBussType()))){
						logger.warn("移动渠道缴费退费,原交易非缴费类交易 :{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
								new Object[]{reqVo.getOriTransactionID(),reqVo.getTransactionID(),transIDH,
								paramData.getMsgSender(),upayLog.getIdValue()});
						log.warn("移动渠道缴费退费,原交易非缴费类交易 :{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
								new Object[]{reqVo.getOriTransactionID(),reqVo.getTransactionID(),transIDH,
								paramData.getMsgSender(),upayLog.getIdValue()});
						 msgRtn.setRspCode(RspCodeConstant.Wzw.WZW_2998.getValue());
						 msgRtn.setRspDesc(RspCodeConstant.Wzw.WZW_2998.getDesc());
					...
					if (CommonConstant.YesOrNo.Yes.toString().equals(upayLog.getRefundFlag())) {
						logger.warn("移动渠道缴费退费,原交易已被退费 :{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
								new Object[]{reqVo.getOriTransactionID(),reqVo.getTransactionID(),transIDH,
								paramData.getMsgSender(),upayLog.getIdValue()});
						log.warn("移动渠道缴费退费,原交易已被退费 :{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
								new Object[]{reqVo.getOriTransactionID(),reqVo.getTransactionID(),transIDH,
								paramData.getMsgSender(),upayLog.getIdValue()});
						msgRtn.setRspCode(RspCodeConstant.Wzw.WZW_2998.getValue());
						msgRtn.setRspDesc(RspCodeConstant.Wzw.WZW_2998.getDesc());
						...
						rspVo.setOriTransactionID(reqVo.getOriTransactionID());
						rspVo.setOriActionDate(reqVo.getOriActionDate());
						rspVo.setRspCode(RspCodeConstant.Crm.CRM_3A15.getValue());
					...
					logger.warn("移动渠道缴费退费,原交易未退费 :{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
							new Object[]{reqVo.getOriTransactionID(),reqVo.getTransactionID(),transIDH,
							paramData.getMsgSender(),upayLog.getIdValue()});
					log.warn("移动渠道缴费退费,原交易未退费 :{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
							new Object[]{reqVo.getOriTransactionID(),reqVo.getTransactionID(),transIDH,
							paramData.getMsgSender(),upayLog.getIdValue()});
					...
					logger.info("移动渠道缴费退费,原交易未被冲正 :{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
							new Object[]{reqVo.getOriTransactionID(),reqVo.getTransactionID(),transIDH,
							paramData.getMsgSender(),upayLog.getIdValue()});
					log.warn("移动渠道缴费退费,原交易未被冲正 :{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
							new Object[]{reqVo.getOriTransactionID(),reqVo.getTransactionID(),transIDH,
							paramData.getMsgSender(),upayLog.getIdValue()});
					...
					logger.info("移动渠道缴费退费,原交易已完成对账 :{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
							new Object[]{reqVo.getOriTransactionID(),reqVo.getTransactionID(),transIDH,
							paramData.getMsgSender(),upayLog.getIdValue()});
					log.warn("移动渠道缴费退费,原交易已完成对账 :{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
							new Object[]{reqVo.getOriTransactionID(),reqVo.getTransactionID(),transIDH,
							paramData.getMsgSender(),upayLog.getIdValue()});
					...
					CrmReverseMsgReqVo reqs = new CrmReverseMsgReqVo();// 报文体
					reqs.setOriReqSys(upayLog.getRcvDomain());// 应该填写原充值的省份(mainOrgId这个可以吧)
					reqs.setOriTransactionID(upayLog.getRcvOprId());
					reqs.setOriActionDate(upayLog.getRcvOprDt());
					...
					logger.info("移动渠道缴费退费,begin 核心向CRM转发返销(冲正)请求流水号:{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
							...
							paramData.getMsgSender(),upayLog.getIdValue()});
					log.warn("移动渠道缴费退费,begin 核心向CRM转发返销(冲正)请求流水号:{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
							new Object[]{transIDH,reqVo.getTransactionID(),transIDH,
							paramData.getMsgSender(),upayLog.getIdValue()});
					...
					logger.info("移动渠道缴费退费,end 核心向CRM转发返销(冲正)请求流水号:{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
							new Object[]{transIDH,reqVo.getTransactionID(),transIDH,
							paramData.getMsgSender(),upayLog.getIdValue()});
					log.warn("移动渠道缴费退费,end 核心向CRM转发返销(冲正)请求流水号:{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
							new Object[]{transIDH,reqVo.getTransactionID(),transIDH,
							paramData.getMsgSender(),upayLog.getIdValue()});

					modifyRcvLs(txnLog, transIDH, intTxnDate, transIDHTime, msg19, mainOrgId, crmMsgVoRtn, reqs);
					logger.info("移动渠道缴费退费,修改添加新交易流水记录:{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
							new Object[]{txnLog.getIntTxnSeq(),reqVo.getTransactionID(),transIDH,
							paramData.getMsgSender(),upayLog.getIdValue()});
					log.warn("移动渠道缴费退费,修改添加新交易流水记录:{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
							new Object[]{txnLog.getIntTxnSeq(),reqVo.getTransactionID(),transIDH,
							paramData.getMsgSender(),upayLog.getIdValue()});
					...
					if (!RspCodeConstant.Crm.CRM_0000.getValue().equals(resVo.getRspCode())) {
						...
						txnLog.setRcvSubRspDesc(CrmErrorCodeCache.getCrmErrCode(errCode));
						txnLog.setChlRspCode(RspCodeConstant.Wzw.WZW_2998.getValue());
						txnLog.setChlRspDesc(RspCodeConstant.Wzw.WZW_2998.getDesc());
						txnLog.setChlRspType(CommonConstant.CrmRspType.BusErr.getValue());
						txnLog.setChlSubRspCode(errCode);
						...
						log.succ("移动渠道缴费退费,CRM应答成功!流水号:{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{},CRM报文体应答码:{},CRM报文体应答描述:{}" ,
								new Object[]{crmMsgVoRtn.getTransIDH(),reqVo.getTransactionID(),transIDH,
								paramData.getMsgSender(),upayLog.getIdValue(),resVo.getRspCode(),resVo.getRspInfo()});
						upayCsysPayLimitService.modifyLimtsDells(upayLog);// 需要先扣除限额表金额(本次退费的金额)
						...
						BankRefundReqVo refundReqVo = new BankRefundReqVo();// 向Bank发起冲正请求报文体
						refundReqVo.setOriReqSys(ExcConstant.BANK_REQ_SYS);// upay向Bank发起缴费
						refundReqVo.setOriReqTransID(upayLog.getIntTxnSeq());// upay向Bank发起缴费流水号
						refundReqVo.setOriReqDate(upayLog.getIntTxnDate());// upay向Bank发起缴费日期
						
						//OriTransType接口文档中没有此字段 by xuyunbo 20131203
						...
						logger.info("移动渠道缴费退费,begin 核心向Bank转发返销(冲正)请求流水号:{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
								new Object[]{transIDH,reqVo.getTransactionID(),transIDH,
								paramData.getMsgSender(),upayLog.getIdValue()});
						log.warn("移动渠道缴费退费,begin 核心向Bank转发返销(冲正)请求流水号:{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
								new Object[]{transIDH,reqVo.getTransactionID(),transIDH,
								paramData.getMsgSender(),upayLog.getIdValue()});
						Map<String, Object> pas = new HashMap<String, Object>();
						BankMsgVo send2Bank = bankRefundBus.execute(msgReq,pas, txnLog, null);
						txnLog.setSettleDate(send2Bank.getReqDate());
						logger.info("移动渠道缴费退费,end 核心向Bank转发返销(冲正)请求流水号:{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
								new Object[]{transIDH,reqVo.getTransactionID(),transIDH,
								paramData.getMsgSender(),upayLog.getIdValue()});
						log.warn("移动渠道缴费退费,end 核心向Bank转发返销(冲正)请求流水号:{},CRM操作流水:{},UPAY流水:{},发起省:{},手机号码:{}" ,
								new Object[]{transIDH,reqVo.getTransactionID(),transIDH,
								paramData.getMsgSender(),upayLog.getIdValue()});
						
						modifyOuterLs(txnLog, send2Bank);
						logger.info("移动渠道缴费退费,修改添加交易流水记录:{},UPAY流水:{},发送银行:{}" ,
								new Object[]{upayLog.getIntTxnSeq(),transIDH,upayLog.getBankId()});
						log.warn("移动渠道缴费退费,修改添加交易流水记录:{},UPAY流水:{},发送银行:{}" ,
								new Object[]{upayLog.getIntTxnSeq(),transIDH,upayLog.getBankId()});
...
//							txnLog.setOuterVersion(ExcConstant.CRM_VERSION);
//							txnLog.setOuterActivityCode(msgReq.getActivityCode());
//							txnLog.setOuterDomain(upayLog.getBankId());// 记录Domain域,机构代码
//							txnLog.setOuterRouteType(msgRtn.getRouteType());
//							txnLog.setOuterRouteVal(msgRtn.getRouteValue());
//							txnLog.setOuterSessionId(msgRtn.getSessionID());
							...
							msgRtn.setRspCode(RspCodeConstant.Wzw.WZW_0000.getValue());
							msgRtn.setRspDesc(RspCodeConstant.Wzw.WZW_0000.getDesc());
							...
							rspVo.setTransactionID(reqVo.getTransactionID());
							rspVo.setSettleDate(send2Bank.getReqDate());// 交易成功时,upss应答必填
							msgRtn.setBody(rspVo);

							modifySuccNewAndOldLog(txnLog, upayLog, logg, msgRtn, msgReq, transCode, msgVo);
							...
							log.warn("移动渠道缴费退费,成功修改新增流水状态:{},UPAY流水:{},发送银行:{},手机号码:{}" ,
									new Object[]{txnLog.getIntTxnSeq(),transIDH,upayLog.getBankId(),upayLog.getIdValue()});
				...
				UpayCsysTxnLogHis logHis = upayCsysTxnLogHisService.findObj(map);
				if (logHis == null) {
...
//		txnLog.setOuterSessionId(msgRtn.getSessionID());
	...
	private void modifyOldLogsStatus(UpayCsysTxnLogHis logHis,UpayCsysTxnLogHis hisLog){
