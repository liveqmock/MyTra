...
//				String reqOprId = Serial.genSerialNo(CommonConstant.Sequence.OprId.getValue());
				...
				forwardMsg.setSessionID(String.valueOf(param.getSeqId())); //
				forwardMsg.setTransIDO(String.valueOf(param.getSeqId()));
...
//				params1.put("organID",String.valueOf(txnLog.getReqDomain()));
				
				if(RspCodeConstant.OrgId.ORGID_0055.getValue().equals(txnLog.getReqDomain())){
					//如果是商城,OrganID =0055和CnlTyp =51（银行网上银行），然后BusiTransID填写银行的流水
					params1.put("organID",txnLog.getReqDomain());
					//busschl 业务渠道
					params1.put("cnlTyp", txnLog.getBussChl());
				}else{
					//如果是省
				...
				if (rtMsg.getBody()==null || rtMsg.getBody().equals("")) {
					log.error("充值失败,返回报文体为空,内部流水:{},发起方:{}",txnSeq, reqMsg.getReqSys());
					logger.error("充值失败,返回报文体为空,内部流水:{},发起方:{}",txnSeq, reqMsg.getReqSys());
					noticeMsg.setRspCode(RspCodeConstant.Crm.CRM_5A02.getValue());
					noticeMsg.setRspInfo(RspCodeConstant.Crm.CRM_5A02.getDesc());
...
//				txnLog.setRcvOprId(reqOprId);
				...
				if (RspCodeConstant.Wzw.WZW_0000.getValue().equals(rtMsg.getRspCode())
					...
					logger.info("{}充值成功,内部流水:{},发起方:{},接收方:{}", new Object[] {
							idValue, txnSeq, reqMsg.getReqSys(),forwardOrg });
