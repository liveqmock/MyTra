...
public class TmallTransResultQueryAction extends AbsBaseAction<BankMsgVo, BankMsgVo>{
	...
	public BankMsgVo execute(BankMsgVo msgVo) throws AppBizException {
		tmallLogger.debug("TmallTransResultQueryAction execute(Object) - start");
		// 请求报文
		...
		try {
			...
			initLog(txnLog, seqId, transIDH, intTxnDate,
					transIDHTime, reqMsg, transCode, resMsg);
			...
			if (!"".equals(StringUtil.toTrim(checkrtn))) {
				tmallOperLogger.error("报文体校验失败:{},内部交易流水:{},发起方:{}", new Object[] {
						checkrtn, transIDH, reqMsg.getReqSys() });
				tmallLogger.debug("报文体校验失败:{},内部交易流水:{},发起方:{}", new Object[] {
						checkrtn, transIDH, reqMsg.getReqSys() });
				...
				txnLog.setTmallRspCode(code);
				txnLog.setTmallRspDesc(RspCodeConstant.Bank.getDescByValue(code));
			...
			if("01".equals(reqBody.getQueryType())){
				if((reqBody.getOriReqTransID() == null) || ("".equals(reqBody.getOriReqTransID()))){
					tmallOperLogger.error("报文体校验失败,原业交易请求流水号（oriReqTransID）不能为空,内部交易流水:{},发起方:{}", new Object[] {
							transIDH, reqMsg.getReqSys() });
					tmallLogger.debug("报文体校验失败,原业交易请求流水号（oriReqTransID）不能为空,内部交易流水:{},发起方:{}", new Object[] {
							transIDH, reqMsg.getReqSys() });
					
					resMsg.setRspCode(RspCodeConstant.Bank.BANK_019A12.getValue());
					resMsg.setRspDesc(RspCodeConstant.Bank.getDescByValue(RspCodeConstant.Bank.BANK_019A12.getValue()));
					resMsg.setRspCode(RspCodeConstant.Bank.BANK_014A04.getValue());
					resMsg.setRspDesc(RspCodeConstant.Bank.BANK_014A04.getDesc());
					resMsg.setBody(resBody);
					
					txnLog.setTmallRspCode(RspCodeConstant.Bank.BANK_019A12.getValue());
					txnLog.setTmallRspDesc(RspCodeConstant.Bank.getDescByValue(RspCodeConstant.Bank.BANK_019A12.getValue()));
					txnLog.setStatus(CommonConstant.TxnStatus.TxnFail.getValue());
					txnLog.setLastUpdTime(DateUtil.getDateyyyyMMddHHmmssSSS());
					upayCsysTmallTxnLogService.modify(txnLog);
					return resMsg;
				...
				if((reqBody.getOriOrderID() == null) || ("".equals(reqBody.getOriOrderID()))){
					tmallOperLogger.error("报文体校验失败，原订单编号（OriOrderID）不能为空,内部交易流水:{},发起方:{}", new Object[] {
							transIDH, reqMsg.getReqSys() });
					tmallLogger.debug("报文体校验失败，原订单编号（OriOrderID）不能为空,内部交易流水:{},发起方:{}", new Object[] {
							transIDH, reqMsg.getReqSys() });
			...
			if(StringUtils.isBlank(forwardOrg)){
				...
				tmallLogger.error("原交易未发送到省充值:{},内部交易流水:{},手机号:{},发起方:{}",
						new Object[] { reqBody.getOriReqTransID(), transIDH, transLog.getIdValue(), reqMsg.getReqSys() });
//				resBody.setOriRspCode(transLog.getTmallRspCode());
//				resBody.setOriRspDesc(transLog.getTmallRspDesc());
				resMsg.setBody(resBody);
				return resMsg;
			}
//			boolean checkFlag = true;// 转发机构交易权限标识
			...
			forwardBody.setOriActionDate(reqBody.getOriReqDate());
			forwardBody.setOriReqSys(transLog.getTmallOrgId());
			...
			txnLog.setCrmOprId(forwardBody.getOriTransactionID());//TODO 该字段set原交易流水? 
...
//			checkFlag = orgStatusCheck(forwardOrg);
			if (checkFlag == null) {
				tmallLogger.debug("sendCrmJmsMessage.sendMsg(forwardMsg) - start,intTxnSeq:{}",new Object[]{transIDH});
				...
				tmallLogger.debug("sendCrmJmsMessage.sendMsg(forwardMsg) - end,intTxnSeq:{}",new Object[]{transIDH});
			
				txnLog.setCrmTranshId(forwardMsg.getTransIDH());
				txnLog.setCrmTranshTm(forwardMsg.getTransIDHTime());
				txnLog.setCrmTranshDt(StrUtil.subString(
				...
				if (forwardMsg.getBody() == null
					...
					tmallLogger.error("天猫充值状态查询,CRM返回报文体为空!内部交易流水:{},发起方:{},接收方:{},手机号:{}",new Object[] { transIDH,
							reqMsg.getReqSys(),forwardMsg.getMsgReceiver(),transLog.getIdValue() });
...
//					resBody.setOriRspDesc(RspCodeConstant.Bank.getDescByValue(errCode));
				...
				if(RspCodeConstant.Upay.UPAY_U99998.getValue().equals(forwardMsg.getRspCode())){
					tmallOperLogger.error("天猫充值状态查询,CRM前置响应超时!内部交易流水:{},发起方:{},接收方:{},手机号:{}",new Object[] { transIDH,
									reqMsg.getReqSys(),forwardMsg.getMsgReceiver() ,transLog.getIdValue()});
					tmallLogger.info("天猫充值状态查询,CRM前置响应超时!内部交易流水:{},发起方:{},接收方:{},手机号:{}",new Object[] { transIDH,
									reqMsg.getReqSys(),forwardMsg.getMsgReceiver(),transLog.getIdValue() });
					...
					upayCsysTmallTxnLogService.modify(txnLog);
//					resBody.setOriRspCode(MessageHandler.getBankErrCode(errCode));
//					resBody.setOriRspDesc(MessageHandler.getBankErrMsg(errCode));
					resMsg.setRspCode(errCode);
					resMsg.setRspDesc(RspCodeConstant.Bank.getDescByValue(errCode));
					resMsg.setBody(resBody);
					return resMsg;
...
//								forwardRtBody.getRspCode())) {
					...
					tmallLogger.info("CRM响应成功!内部交易流水:{},发起方:{},接收方:{},手机号:{}",
							new Object[] { transIDH,reqMsg.getReqSys(),forwardMsg.getMsgReceiver(),transLog.getIdValue() });
				...
				if (RspCodeConstant.Crm.CRM_2998.getValue().equals(forwardMsg.getRspCode())){
					tmallOperLogger.error("天猫充值状态查询,CRM响应失败!内部交易流水:{},发起方:{},接收方:{},手机号:{}",
							new Object[] { transIDH,reqMsg.getReqSys(),forwardMsg.getMsgReceiver(),transLog.getIdValue()});
					tmallLogger.error("天猫充值状态查询,CRM响应失败!内部交易流水:{},发起方:{},接收方:{},手机号:{}",
							new Object[] { transIDH,reqMsg.getReqSys(),forwardMsg.getMsgReceiver(),transLog.getIdValue() });
...
//					resBody.setOriResultCode("");
//					resBody.setOriResultDesc("");
				...
				txnLog.setLastUpdTime(DateUtil.getDateyyyyMMddHHmmssSSS());
				upayCsysTmallTxnLogService.modify(txnLog);
				tmallLogger.debug("TmallTransResultQueryAction execute(Object) - end");
				return resMsg;
			} else {
				
				tmallOperLogger.warn("天猫充值状态查询,落地方机构状态异常:{},内部交易流水:{},发起方:{},接收方:{},手机号:{}",new Object[] { forwardOrg, transIDH,
						reqMsg.getReqSys(),forwardMsg.getMsgReceiver(),transLog.getIdValue() });
				tmallLogger.warn("天猫充值状态查询,落地方机构状态异常:{},内部交易流水:{},发起方:{},接收方:{},手机号:{}",new Object[] { forwardOrg, transIDH,
						reqMsg.getReqSys(),forwardMsg.getMsgReceiver(),transLog.getIdValue() });
...
//						+ "接收方" + forwardOrg + "交易权限关闭");
			...
			tmallLogger.error("天猫充值状态查询,运行异常,错误代码:{},内部交易流水号:{},业务发起方:{}",
					new Object[] { errCode,transIDH, reqMsg.getReqSys() });
			tmallLogger.error("运行异常:",e);
			txnLog.setStatus(CommonConstant.TxnStatus.TxnFail.getValue());
			txnLog.setLastUpdTime(DateUtil.getDateyyyyMMddHHmmssSSS());
			txnLog.setTmallRspCode(errCode);
			txnLog.setTmallRspDesc(RspCodeConstant.Bank.getDescByValue(errCode));
...
//			resBody.setOriRspDesc(RspCodeConstant.Bank.getDescByValue(errCode));
			...
			tmallOperLogger.error("天猫充值状态查询,业务异常!内部交易流水号:{},业务发起方:{}",
					new Object[] {RspCodeConstant.Bank.getDescByValue(errCode),transIDH, reqMsg.getReqSys() });
			tmallLogger.error("天猫充值状态查询,业务异常,代码:{},内部交易流水号:{},业务发起方:{}",
					new Object[] { errCode,transIDH, reqMsg.getReqSys() });
			tmallLogger.error("天猫充值状态查询,业务异常:",e);
			txnLog.setStatus(CommonConstant.TxnStatus.TxnFail.getValue());
			txnLog.setTmallRspCode(errCode);
			txnLog.setTmallRspDesc(RspCodeConstant.Bank.getDescByValue(errCode));
			txnLog.setLastUpdTime(com.huateng.toolbox.utils.DateUtil.getDateyyyyMMddHHmmssSSS());
...
//			resBody.setOriRspDesc(RspCodeConstant.Bank.getDescByValue(errCode));
			...
			tmallLogger.error("天猫充值状态查询,系统异常!内部交易流水号:{},业务发起方:{}",
					new Object[] {transIDH, reqMsg.getReqSys() });
			tmallLogger.error("天猫充值状态查询,系统异常:",e);
			txnLog.setStatus(CommonConstant.TxnStatus.TxnFail.getValue());
			txnLog.setTmallRspCode(errCode);
