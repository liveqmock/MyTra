...
public class BankReverseAction extends AbsBaseAction<BankMsgVo, BankMsgVo> {
	...
	public BankMsgVo execute(BankMsgVo paramData) throws AppBizException {
		...
		try {
			...
			logger.warn("begin 添加Bank渠道冲正交易流水:{} ,内部交易流水号intTxnSeq:{},发起银行:{}", 
					new Object[]{bankMsgVo.getReqTransID(), transIDH,bankMsgVo.getReqSys()});
			log.warn("begin 添加Bank渠道冲正交易流水:{} ,内部交易流水号intTxnSeq:{},发起银行:{}", 
					new Object[]{bankMsgVo.getReqTransID(), transIDH,bankMsgVo.getReqSys()});
			logger.debug("begin 添加银行渠道冲正交易流水,intTxnSeq:{}",new Object[]{transIDH});
			initLog(txnLog, seqValue, transIDH, intTxnDate, transIDHTime, bankMsgVo, transCode, msgVoRtn, reqMsg);
			logger.warn("end 添加Bank渠道冲正交易流水:{} ,内部交易流水号intTxnSeq:{},发起银行:{}", 
					new Object[]{bankMsgVo.getReqTransID(), transIDH,bankMsgVo.getReqSys()});
			log.warn("end 添加Bank渠道冲正交易流水:{} ,内部交易流水号intTxnSeq:{},发起银行:{}", 
					new Object[]{bankMsgVo.getReqTransID(), transIDH,bankMsgVo.getReqSys()});
...
//			boolean orgFlag = orgStatusCheck(mainOrgId);// 归属省
			...
			if (CommonConstant.YesOrNo.Yes.getValue().equals(upayLog.getReverseFlag())) {
				...
				logger.warn("原交易未冲正:{}, Bank操作流水:{} ,内部交易流水号intTxnSeq:{},发起银行:{},手机号码:{}", 
					new Object[]{reqMsg.getOriReqTransID(), bankMsgVo.getReqTransID(), transIDH,
					bankMsgVo.getReqSys(),upayLog.getIdValue()});
				log.warn("原交易未冲正:{}, Bank操作流水:{} ,内部交易流水号intTxnSeq:{},发起银行:{},手机号码:{}", 
						new Object[]{reqMsg.getOriReqTransID(), bankMsgVo.getReqTransID(), transIDH,
						bankMsgVo.getReqSys(),upayLog.getIdValue()});
...
//				txnLog.setIntTxnSeq(reqVo.getTransactionID());
				...
				logger.warn("begin 核心向CRM转发返销(冲正)请求流水号:{}, Bank操作流水:{} ,内部流水:{},发起银行:{},手机号码:{}", 
						new Object[]{transIDH, bankMsgVo.getReqTransID(), transIDH,
						bankMsgVo.getReqSys(),upayLog.getIdValue()});
				log.warn("begin 核心向CRM转发返销(冲正)请求流水号:{}, Bank操作流水:{} ,内部流水:{},发起银行:{},手机号码:{}", 
						new Object[]{transIDH, bankMsgVo.getReqTransID(), transIDH,
						bankMsgVo.getReqSys(),upayLog.getIdValue()});
				Map<String, Object> params = new HashMap<String, Object>();
				CrmMsgVo crmMsgVoRtn = crmReverseBus.execute(msg19, params,txnLog, null);
				logger.warn("end 核心向CRM转发返销(冲正)请求流水号:{}, Bank操作流水:{} ,内部流水:{},发起银行:{},手机号码:{}", 
						new Object[]{transIDH, bankMsgVo.getReqTransID(), transIDH,
						bankMsgVo.getReqSys(),upayLog.getIdValue()});
				log.warn("end 核心向CRM转发返销(冲正)请求流水号:{}, Bank操作流水:{} ,内部流水:{},发起银行:{},手机号码:{}", 
						new Object[]{transIDH, bankMsgVo.getReqTransID(), transIDH,
						bankMsgVo.getReqSys(),upayLog.getIdValue()});
...
//					msgVoRtn.setBody(CommonConstant.SpeSymbol.BLANK.toString());
					...
					logger.info("修改原纪录及新增交易流水:{}, Bank操作流水:{} ,内部交易流水号intTxnSeq:{},发起银行:{},手机号码:{}", 
							...
							bankMsgVo.getReqSys(),upayLog.getIdValue()});
					log.succ("修改原纪录及新增交易流水:{}, Bank操作流水:{} ,内部交易流水号intTxnSeq:{},发起银行:{},手机号码:{}", 
							new Object[]{txnLog.getIntTxnSeq(), bankMsgVo.getReqTransID(), transIDH,
							bankMsgVo.getReqSys(),upayLog.getIdValue()});
					...
					logger.warn("修改新增交易流水:{}, Bank操作流水:{} ,内部流水:{},发起银行:{}'", 
							new Object[]{txnLog.getIntTxnSeq(), bankMsgVo.getReqTransID(), transIDH,
							bankMsgVo.getReqSys()});
					log.warn("修改新增交易流水:{}, Bank操作流水:{} ,内部流水:{},发起银行:{}", 
							new Object[]{txnLog.getIntTxnSeq(), bankMsgVo.getReqTransID(), transIDH,
							bankMsgVo.getReqSys()});
