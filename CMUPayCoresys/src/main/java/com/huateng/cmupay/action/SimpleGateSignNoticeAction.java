...
public class SimpleGateSignNoticeAction extends
	...
	public CoreResultSignRes execute(Object param) throws AppBizException {
		...
		try {
			...
			resMsg.setTransactionID(logTmp.getReqTransId());
			...
			if (CommonConstant.TxnStatus.TxnSuccess.getValue().equals(logTmp.getStatus())) {
				logger.info("网关签约结果通知,网关签约后台结果通知为成功,订单号:{},签约协议号:{}",
						new Object[] {reqMsg.getOrderID(), reqMsg.getSubID()});
				log.succ("网关签约结果通知,网关签约后台结果通知为成功,订单号:{} ,签约协议号:{}",
						new Object[] {reqMsg.getOrderID(), reqMsg.getSubID()});
			} else {
				logger.error("网关签约结果通知,网关签约后台结果通知为失败,或者未通知,订单号:{} ,签约协议号:{}",
						new Object[] {reqMsg.getOrderID(), reqMsg.getSubID()});
				log.error("网关签约结果通知,网关签约后台结果通知为失败 ,或者未通知,订单号:{},签约协议号:{}",
						new Object[] {reqMsg.getOrderID(), reqMsg.getSubID()});
			}
